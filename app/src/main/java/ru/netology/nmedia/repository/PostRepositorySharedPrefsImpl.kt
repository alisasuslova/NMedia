...
class PostRepositorySharedPrefsImpl(
    ...
    companion object {
        private const val KEY = "posts" // ключ по которому вся строка с постами будет храниться
    }
    ...
    init {
        ...
        data.value = posts // в любом случае обновляем значение
    }
    private fun sync() { // обращаемся к prefs  и редактируем. with - последовательное выполнение действий на объекте
        with(prefs.edit()) {
            putString(KEY, gson.toJson(posts)) // строка по ключу, выдаст список постов в формате .json!
            apply()
        }
